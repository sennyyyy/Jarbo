...
Jarbo is a regular Dock and Command-Tab app while also remaining available in the menu bar. The menu-bar control can show or hide the HUD, open control configuration, pause or resume hand controls, display Core ML status, and quit Jarbo.
...
- The training panel reports landmark readiness for each hand. Wait for `21/21`, hold the full selected hand in frame, then add the pose sample. Configuration mode keeps landmark capture active while pausing gesture actions and expensive live classification.
...
Create `release-notes/v1.0.8.md` with **Working** and **Known limitations** sections, then publish with:
